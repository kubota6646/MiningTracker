...
### v2.2.0 - 重要なバグ修正とメンテナンスリリース (2026-02-03)
- **重大なバグ修正**: DatabaseManagerのResultSetリソースリークを修正
- 30箇所のResultSetオブジェクトがtry-with-resourcesで適切に閉じられるように修正
- リソースリークによるメモリ枯渇とコネクションプール枯渇の問題を解消
- バージョン番号の一貫性を確保（build.gradle、CHANGELOG、VERSION.mdを統一）
- コードの品質とパフォーマンスが向上
